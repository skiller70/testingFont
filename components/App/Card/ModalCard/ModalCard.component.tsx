  ...
  return (
    <Dialog.Portal>
      ...
      >
        ...
        >
          <Dialog.Title className={`capitalize ${styles.title}`}>
            {modalTitle}
          </Dialog.Title>
          <Dialog.Close asChild>
            <button aria-label="Close">
              <XMarkIcon className={`w-8 h-8 ${styles.close}`} />
            </button>
          </Dialog.Close>
        </div>
        <div className="p-4">
          {backgroundText && (
            <>
              <h4 className={`font-medium ${styles.heading}`}>Background:</h4>
              <p className={`font-light ${styles.copy}`}>{backgroundText}</p>
            </>
          )}

          {futureUseText && (
            <>
              <h4 className={`font-medium ${styles.heading}`}>Future Use:</h4>
          ...
          {skills.length > 0 && (
            <>
              ...
              <p className={`font-light ${styles.copy}`}>{formattedSkills}</p>
            </>
          )}
          <Link
